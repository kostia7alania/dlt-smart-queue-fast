...
- Отличаются ли `tyw_id` для New / Renew? Да: например, для office 47 живой API
  2026-07-19 вернул `111093` для ` NEW THAI` и `111095` для ` RENEW THAI`.
- Можно ли различать `tyw_id` по Motorcycle / car / Car and Motocycle? В текущем
  наблюдаемом контракте — нет: vehicle не входит в payload `workfilter`, а
  `getVehicle` 2026-07-19 вернул одинаковый список для `ve_type=1` и `ve_type=2`.
  Не добавлять vehicle-фильтр календаря, пока upstream не даст новый
  параметр или различимые work types.
