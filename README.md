...
An independent guide to Thai driving-licence journeys, preparation and office
choice, with DLT appointment discovery, maps, comparisons and stored history.
It helps people understand their next step; it does not book appointments,
...
- Static content routes: 20 licence/process pages under `/licence`, eight area
  hubs, `/offices/all`, 206 office detail pages and evidence guides. Statements
  distinguish observed data, DLT-only decisions and dated third-party reports.
...
- licence journeys: <http://localhost:3000/licence>
- all offices: <http://localhost:3000/offices/all>
...
```bash
node tools/build-office-directory.mjs           # rebuild from committed inputs
node tools/build-office-directory.mjs --check   # check reproducibility, not live freshness
...
To run PostgreSQL integration tests locally (otherwise they are skipped):
...
These show request shapes with historical example IDs/dates. Resolve current
office/work-type IDs and a suitable request date before checking availability.
...
The historical market research, keyword hypotheses, measurement plan, and
superseded domain shortlist are documented in
